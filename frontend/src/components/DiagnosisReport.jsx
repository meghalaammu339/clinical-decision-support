...
//         <RiskScore riskData={data.risk_assessment} />
...
//       <DrugInteractions medications={structured_case?.current_medications} />
//       <FollowUpChat report={final_report} />
    ...
    <div className="report">
      ...
      <div className="report-header">
        ...
        <div className="report-header-actions">
          <ExportButton data={data} />
          <div className="urgency-badge" style={{ background: URGENCY_COLOR[urgency] }}>
            {urgency.toUpperCase()}
          </div>
        </div>
      </div>
            ...
            {differentials.map((dx, i) => (
              <div key={i} className="diff-card">
                <div className="diff-header">
                  ...
                  <span className="diff-urgency" style={{
                    borderColor: URGENCY_COLOR[dx.urgency],
                    color: URGENCY_COLOR[dx.urgency]
                  }}>
                    {dx.urgency}
                ...
                <div className="diff-evidence">
                  <div>
                    <p className="evidence-label for">✓ Supporting</p>
                    <ul>{dx.supporting_evidence?.map((e, j) => <li key={j}>{e}</li>)}</ul>
                  </div>
                  <div>
                    <p className="evidence-label against">✗ Against</p>
                    <ul>{dx.against_evidence?.map((e, j) => <li key={j}>{e}</li>)}</ul>
                  </div>
                </div>
                <div className="diff-footer">
                  <span>📚 {dx.cited_source}</span>
      ...
      {final_report?.immediate_actions?.length > 0 && (
        <div className="section">
          <h3>Immediate Actions</h3>
          <ol className="action-list">
            {final_report.immediate_actions.map((a, i) => <li key={i}>{a}</li>)}
          </ol>
        </div>
      )}
